...
export const LocationModal: React.FC<LocationModalProps> = ({ 
  ...
  const handleRecentSelect = (recent: {district: string, constituency?: string}) => {
    // Close modal immediately
      ...
      setTimeout(() => {
        onLoadingChange?.(true);
      saveToRecentSearches(recent.district, recent.constituency || undefined);
        
        // Simulate loading delay
        setTimeout(() => {
        onSelect(recent.district, recent.constituency || undefined);
          onLoadingChange?.(false);
        }, 1500);
      }, 100);
  };
  ...
  const handleClearDistrict = () => {
    ...
    // Auto-focus the input field after clearing
    setTimeout(() => {
      inputRef.current?.focus();
    }, 0);
  };

  const handleClearConstituency = () => {
    setTempSelectedConstituency('');
  };
          ...
          {tempSelectedDistrict && availableConstituencies.length > 0 && (
            <div className="px-4 pb-6 border-b border-gray-100">
              <div className="mb-4 flex items-center justify-between">
                ...
                {tempSelectedConstituency && (
                  <button
                    onClick={handleClearConstituency}
                    className="text-sm text-gray-500 hover:text-gray-700 font-normal transition-colors underline"
                  >
                    क्लिअर
                  </button>
                )}
